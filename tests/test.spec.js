...
test('home page', async ({ page }) => {
	await page.goto('/');
	expect(await page.textContent('h1')).toBe('Govind Kamtamneni');
	const locator = page.locator('a.button.primary');
	await expect(locator).toBeVisible();
	await expect(locator).toHaveAttribute('href', 'https://github.com/thegovind');
});

test('about page', async ({ page }) => {
	await page.goto('/about');
	expect(await page.textContent('h1')).toBe("Hi, I'm Govind!");
	const locator = page.locator('#memoji');
	await expect(locator).toHaveAttribute('alt', "Govind's Memoji");
